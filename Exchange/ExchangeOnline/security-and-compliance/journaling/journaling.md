...
<sup>1</sup>In case sender and recipients are both in the accepted domain list of the tenant, these messages are not honored as External even if the `x-ms-exchange-crosstenant-authas` header has the value `anonymous`. Accordingly these messages are not journaled as External.
