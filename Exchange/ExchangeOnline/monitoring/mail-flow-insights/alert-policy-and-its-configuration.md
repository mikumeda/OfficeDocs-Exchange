...
These licenses determine the features/functionalities which users and administrators in the organization can have access to. These functionalities are the ones for which the user/administrator can create alert policies.
...
System policy is created by the system, by default; hence, it's also referred to as "default alert policy".
...
- Turn it off (by default, it's turned on).
...
Custom policy is the policy that can be manually created by the user.

## Privileges required for alert policy management

The following privileges are associated with alert policies management:
...
- **Security reader**:  This privilege is required if the user wants to just read/view an alert policy.
...
- **Creation**: A user can create an alert policy, which is a custom alert policy. For information on how to create an alert policy, see [Create custom policy](#create-custom-policy). 
...
2. In the left pane, select **Mail flow > Alert policies**, and select **New alert policy**. 
3. Provide a name for your policy in the **Name** box and select **Next**.
...
5. From the **Trigger an alert when the following insight is generated** drop-down list, select one of the following types of insights:
...
9. Select **Next**.
10. Review the alert-policy settings and select **Create**.
...
**Categorized under custom policies**
...
The procedure to edit an alert policy is the same for all types of **custom** policies. For more information on how to edit a custom policy, see [Edit custom policy](#edit-custom-policy).

**Categorized under system policies**
...
To edit a custom policy (of any type), perform the following steps:

1. On the left navigation pane of the new EAC, select **Mail flow** > **Alert policies**.
1. Select the policy that you want to edit, and select it.
...
This section describes the procedure to edit the system policies:

- **Messages for priority accounts are delayed or rejected**: This type of system policy can be edited in two methods. For more information, see [First method](#first-method) and [Second method](#second-method).
- **Messages for normal accounts are delayed**: This type of system policy can be edited in the procedure identical to the one implemented for the type **Messages for priority accounts are delayed or rejected**. For more information, see [First method](#first-method).

There are two methods by which a user can edit a system policy.
...
1. On the screen that appears, select **Edit policy**.
...
1. Select the alert policy you want to disable and select it.
...
An alert policy (**System** or **Custom**) is displayed on the **Alerts** screen for a user to view.
...
3. Under the **Alert name** column, select the alert for which you want to view the details.
   The alert policy details screen appears.
